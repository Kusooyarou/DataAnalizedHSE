...
def generate_text_reports() -> None:
    """
    ...
    try:
        ...
        matches_count_per_club = matches_normalized.groupby('home_club_id').size().reset_index(name='matches_count')
        matches_count_per_club = matches_count_per_club.merge(clubs_normalized[['club_id', 'club_name']],
                                                              left_on='home_club_id', right_on='club_id', how='left')
        matches_count_per_club = (matches_count_per_club[['club_name', 'matches_count']].
                                  sort_values(by='matches_count', ascending=False).drop_duplicates())
        print("Отчет о количестве матчей, проведенных каждым клубом:")
        print(matches_count_per_club.head())

        # Отчет о тренерах и количестве проведенных ими матчей
        matches_count_per_coach = (matches_normalized.groupby('home_club_manager_name').
                                   size().reset_index(name='matches_count'))
        matches_count_per_coach = (matches_count_per_coach[['home_club_manager_name', 'matches_count']].sort_values
                                   (by='matches_count', ascending=False).drop_duplicates())
        print("\nОтчет о количестве матчей, проведенных каждым тренером:")
        print(matches_count_per_coach.head())
